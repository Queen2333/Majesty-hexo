...
#### Composition API
  ...
  setup 函数：这是 Composition API 的核心，所有的响应式数据和逻辑都在这个函数内定义。
  reactive：用于创建响应式对象。
  ref：用于创建响应式的基本数据类型（如字符串、数字等）。
  computed：计算属性，用于派生和缓存数据。
  watch：用于监听响应式数据的变化。
  provide 和 inject：用于跨组件传递数据，替代了 Vue 2 中的 props 和 event bus。

  Vue 3 提供了两种开发方式：

  Options API：这是 Vue 2 中的常规方式，通过 data、methods、computed、watch 等选项定义组件。
  Composition API：通过 setup 函数、reactive、ref 等 API 以函数的形式来组织组件逻辑。适用于逻辑复用和代码组织。

---

#### 更好的 TypeScript 支持

  Vue 3 对 TypeScript 的支持做了显著改进，组件和响应式 API 都可以更好地与 TypeScript 集成。Vue 3 使用了全新的 TypeScript 类型定义，增强了类型推导和静态检查。

  TypeScript 支持更强：defineComponent，ref，reactive 等都有更清晰的类型定义。
  更好的类型推导：使用 Composition API 时，TypeScript 可以更好地推断组件状态的类型。

---

#### 性能优化

  Vue 3 进行了很多性能优化，主要体现在以下方面：

  虚拟 DOM 的优化：Vue 3 在虚拟 DOM 渲染时减少了不必要的操作，提升了性能。
  Tree-shaking：Vue 3 完全支持 Tree-shaking，这意味着你只会引入你实际使用的功能，从而减少最终构建的包大小。
  更快的响应式系统：Vue 3 使用了 Proxy API 来替代 Vue 2 中的 Object.defineProperty，使得响应式系统更快、效率更高。

---

#### Fragment

  在 Vue 2 中，组件必须只有一个根元素，而 Vue 3 引入了 Fragment，允许一个组件返回多个根节点元素。

  多个根节点：Vue 3 支持组件可以有多个根节点，无需包装在一个父节点中。

---

#### Teleport

Teleport 是 Vue 3 新增的功能，允许开发者将子组件的 DOM 元素传送到 DOM 树中的其他位置。这在实现弹窗、模态框、通知等场景时非常有用。

```<teleport>``` 标签：使用 ```<teleport>``` 标签可以将一个组件的内容渲染到 DOM 树的其他位置。

---

#### Suspense 和异步组件

  Vue 3 引入了 Suspense，它用于处理异步组件加载，类似于 React 中的 Suspense。

  异步组件：通过 defineAsyncComponent 来定义异步组件。
  Suspense：包裹异步组件，直到异步组件加载完成才渲染其内容。

___

#### Emits Options

  在 Vue 3 中，可以通过 emits 选项显式声明组件会触发的事件。这对于 TypeScript 类型推导和开发体验有很大帮助。
...
#### 新生命周期钩子
  ...
  onMounted、onUpdated、onUnmounted：这几个生命周期钩子用于替代 Vue 2 中的 mounted、updated 和 beforeDestroy 等。

---

#### 全新的响应式系统

  Vue 3 用 Proxy 替代了 Vue 2 中的 Object.defineProperty 来实现响应式系统。这个改进带来了：

  更好的性能和支持更多的数据类型。
  支持更复杂的操作，如数组和对象的动态添加和删除。
  支持更精确的性能优化（例如：懒加载和更少的副作用）。

---

#### Vite 和 Vue 3 的集成

  Vue 3 推荐与 Vite 搭配使用，Vite 是一个下一代的构建工具，具有快速的热更新、非常高效的构建速度和现代的开发体验。

  Vite 提供了对 Vue 3 的完美支持，尤其是在开发模式下，它的性能表现非常优秀。

---

#### 自定义指令（Directives）

  Vue 3 保留了 v-bind、v-for、v-if 等指令，并支持用户定义自定义指令，增强了模板的灵活性和功能。

---

#### 更好的异步和并发控制

  Vue 3 引入了 Suspense 和 async 组件处理，使得在渲染异步数据时更加灵活。
...
#### 其他新特性
  ...
  ```js
    <script setup>：这是 Vue 3.2 引入的一个更简化的语法，帮助开发者用更简洁的方式定义组件，减少模板中的样板代码。
    useContext：Composition API 中的 useContext 允许访问上下文信息，减少代码重复。
    createApp：Vue 3 的应用实例通过 createApp 方法创建，替代了 Vue 2 中的 new Vue()。
  ```
